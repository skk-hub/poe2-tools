    ...
    if (url.pathname === "/api/ocr" && req.method === "POST") {
      ...
      try {
        ...
        await new Promise((resolve, reject) =>
          exec(`convert "${tmpIn}" -resize 400% -colorspace gray -normalize -bordercolor white -border 20x20 "${tmpProc}"`,
            (err, _, stderr) => err ? reject(new Error(stderr || err.message)) : resolve())
        );
        const text = await new Promise((resolve, reject) =>
          exec(`tesseract "${tmpProc}" "${tmpBase}" --oem 1 --psm 4 -c preserve_interword_spaces=1`,
            (err, _, stderr) => {
