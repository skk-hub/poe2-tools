...
if (require.main === module) {
  ...
  // `status: verified` means "mechanics and identifiers checked against authoritative data" — it
  // does NOT mean anyone ever documented this craft. The amulet fixture is verified with
  // provenance.source_url = null and a self-referential source_title ("poe2-kb extraction"),
  // while the boots/bow recipes cite real page reads. Both shipped as plain "verified", so a
  // cited recipe and an in-house construction looked identical to the caller — in a KB whose
  // whole premise is being cited. So publish the distinction.
  // DERIVED, and kept OUT of the recipe docs (the schema is additionalProperties:false and the
  // authored documents must stay schema-clean): a citation is just a source_url that exists,
  // so read it off provenance rather than trusting a field someone must remember to flip.
  const sourced = {};
  for (const [id, doc] of Object.entries(recipes)) sourced[id] = !!(doc.provenance && doc.provenance.source_url);
  const out = { generated: new Date().toISOString().replace(/\.\d+Z$/, "Z"), source: "poe2-kb/crafting/recipes", recipes, sourced };
