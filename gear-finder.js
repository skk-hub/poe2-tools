  ...
  function renderOptResults(d) {
    ...
    return head + d.results.map((r, i) => `
      <div class="gf-optcard${i === 0 ? " best" : ""}">
        ...
        <table class="gf-opttbl"><tbody>${r.picks.map((p) => { const link = !p.keep && p.base && p.account; return `<tr${link ? ` class="gf-opt-link" role="link" tabindex="0" data-base="${esc(p.base)}" data-account="${esc(p.account)}" data-mods="${esc(JSON.stringify(p.mods || []))}" title="open this listing on the trade site"` : ""}><td>${esc(slotLabel(p.slot))}</td><td>${p.keep ? "<span class='muted'>keep " + (/^jewel\d+$/.test(p.slot) && p.name && p.name !== "(keep current)" ? esc(p.name) : "current") + "</span>" : "<b>" + esc(p.name || "item") + "</b>"}</td><td>${p.keep ? "" : deltaSpan(p.dDPS, "DPS")}</td><td>${p.keep ? "" : fmt(p.priceDiv) + " div"}</td></tr>`; }).join("")}</tbody></table>
      </div>`).join("");
  }
