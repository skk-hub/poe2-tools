...
- Verification: `node --check server.js` passed. Restarted the local `server.js` process from this checkout, verified the empty state shows All filters with blank Min count and no Min ilvl, then imported a chest item and confirmed the default filters auto-filled with current values (`Evasion 900`, `Life 120`, `Total elemental resistance 67`, etc.) for the selected item. Live Trade2 chest search returned 4 listings and compared candidates against current item values through the shared queue with no console errors.
