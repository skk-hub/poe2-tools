...
- Verification: `node --check server.js` passed. `git status --short` ended clean after the user fixed local `.git` permissions/ran Git locally, but this session still could not reliably run browser/server/Git helper commands due to intermittent Windows runner `CreateProcessAsUserW failed: 1312`.
- Next: start Codex with `codex --sandbox workspace-write --ask-for-approval on-request` from the repo, run `node server.js`, browser-test Gear Search with real copied gear and a live Trade2 search, verify query preview shows `equipment_filters.filters.dps/es/ev`, then push if not already pushed.
