...
- 2026-06-11: Added global critical chance (`critChance`, `explicit.stat_587431675`) to Gear Search glove filters after verifying gloves can roll/search it; attack-specific crit chance still is not exposed for gloves because Trade2 returned zero glove results for that stat.
...
- Verification: Queried live Trade2 for gloves with `explicit.stat_587431675` and got results; queried gloves with attack-specific crit `explicit.stat_2194114101` and got zero. `node --check server.js` passed, restarted `server.js`, and confirmed `/api/gear-search/analyze` returns `critChance` in `slots.gloves.statKeys`.
...
- Next: continue Gear Search hardening with more imported item shapes and edge-case browser exports.
